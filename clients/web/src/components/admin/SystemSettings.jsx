...
export default SystemSettings;, []);
  ...
  const handleResetAll = async () => {
    if (window.confirm('¿Estás seguro de restablecer todas las configuraciones a valores por defecto?')) {
      try {
        setSaving(true);
  ...
  const renderSettingControl = (setting) => {
    ...
    if (typeof currentValue === 'boolean') {
      ...
      if (key.includes('type') || key.includes('mode') || key.includes('quality')) {
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start py-4 border-b border-gray-700 last:border-0">
        <div>
          <label htmlFor={key} className="block text-sm font-medium text-text-primary">
            {key}
          </label>
          {description && (
        ...
        <Card>
          <div className="animate-pulse">
            <div className="flex space-x-4 mb-6">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="h-8 bg-background-dark rounded w-24"></div>
              ))}
            </div>
