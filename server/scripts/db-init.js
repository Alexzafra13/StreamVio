...
const db = new sqlite3.Database(dbPath, (err) => {
  ...
  db.serialize(() => {
    ...
    try {
      ...
      // Función asíncrona para crear usuario admin
      const createAdminUser = async () => {
        try {
          ...
          // Usar una promesa para manejar la inserción
          await new Promise((resolve, reject) => {
            db.run(
              ...
              (err) => {
                if (err) {
                  console.error("Error detallado al crear usuario admin:", err);
          ...
          console.error("Error completo en createAdminUser:", error);
          throw error;
        }
      };
