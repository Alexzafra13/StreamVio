...
router.get('/', async (req, res) => {
  try {
    const libraries = await db.asyncAll('SELECT * FROM libraries ORDER BY name');
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const scanAutomatically = scan_automatically === undefined ? true : !!scan_automatically;
    ...
    const newLibrary = await db.asyncGet('SELECT * FROM libraries WHERE id = ?', [libraryId]);
...
router.put('/:id', async (req, res) => {
  ...
  // Validar los datos
  if (!name && !libraryPath && type === undefined && scan_automatically === undefined) {
    return res.status(400).json({ 
      error: 'Datos incompletos',
  ...
  try {
    ...
    await db.asyncRun(
      `UPDATE libraries SET ${fields} WHERE id = ?`,
      params
    );
    
    // Obtener la biblioteca actualizada
    const updatedLibrary = await db.asyncGet('SELECT * FROM libraries WHERE id = ?', [libraryId]);
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await db.asyncRun('DELETE FROM media_items WHERE library_id = ?', [libraryId]);
...
router.post('/:id/scan', async (req, res) => {
  ...
  try {
    ...
    console.error(`Error al iniciar el escaneo de la biblioteca ${libraryId}:`, error);
    res.status(500).json({ 
      error: 'Error del servidor',
...
router.get('/:id/media', async (req, res) => {
  ...
  // Validar orden
  const validSortFields = ['title', 'duration', 'size', 'created_at', 'updated_at'];
  ...
  try {
    ...
    console.error(`Error al obtener elementos de la biblioteca ${libraryId}:`, error);
    res.status(500).json({ 
      error: 'Error del servidor',
