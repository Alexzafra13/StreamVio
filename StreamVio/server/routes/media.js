...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const mediaItem = await db.asyncGet('SELECT * FROM media_items WHERE id = ?', [mediaId]);
    ...
    if (mediaItem.library_id) {
      const library = await db.asyncGet('SELECT name, type FROM libraries WHERE id = ?', [mediaItem.library_id]);
      if (library) {
    ...
    if (mediaItem.parent_id) {
      const parentItem = await db.asyncGet('SELECT id, title FROM media_items WHERE id = ?', [mediaItem.parent_id]);
      if (parentItem) {
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(updates).length <= 1) { // Solo updated_at
    ...
    const updatedMediaItem = await db.asyncGet('SELECT * FROM media_items WHERE id = ?', [mediaId]);
    ...
    console.error(`Error al actualizar el elemento multimedia ${mediaId}:`, error);
    res.status(500).json({ 
      error: 'Error del servidor',
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (mediaItem.type === 'series') {
      await db.asyncRun('DELETE FROM media_items WHERE parent_id = ?', [mediaId]);
    }
    ...
    console.error(`Error al eliminar el elemento multimedia ${mediaId}:`, error);
    res.status(500).json({ 
      error: 'Error del servidor',
