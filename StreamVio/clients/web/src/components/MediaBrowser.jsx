...
function MediaBrowser({ libraryId = null, type = null, searchTerm = null }) {
  ...
  const fetchMedia = async (page = 1) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/media?${params.toString()}`, {
        headers: {
      ...
      setError(err.response?.data?.message || "Error al cargar los elementos multimedia");
      setLoading(false);
    }
  };
  ...
  const getThumbnail = (item) => {
    if (item.thumbnail_path && item.thumbnail_path.startsWith('http')) {
      ...
      return `${API_URL}/thumbnails/${item.thumbnail_path.split('/').pop()}`;
    }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      {media.length === 0 ? (
        <div className="bg-gray-800 rounded-lg p-8 text-center">
          <p className="text-gray-400 mb-4">No se encontraron elementos multimedia.</p>
            ...
            {media.map(item => (
              <div key={item.id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
                ...
                <div className="p-4">
                  ...
                  >
                    {item.type === 'movie' || item.type === 'episode' ? 'Reproducir' : 
                     item.type === 'photo' ? 'Ver' : 'Escuchar'}
                  </a>
                </div>
              </div>
            ))}
